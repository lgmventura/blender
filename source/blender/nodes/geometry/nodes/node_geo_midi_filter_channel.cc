...
namespace blender::nodes::node_geo_midi_filter_channel_cc {
...
static void node_declare(NodeDeclarationBuilder &b)
{
  ...
  b.add_input<decl::Int>("Channel"_ustr)
      ...
      .min(0)
      .max(15)
      .description("MIDI channel to keep (0-15)");
  b.add_output<decl::Geometry>("Events"_ustr)
      .description("Point cloud containing only events from the selected channel");
}

static void node_geo_exec(GeoNodeExecParams params)
{
  GeometrySet geometry_set = params.extract_input<GeometrySet>("Events"_ustr);
  const int channel = params.extract_input<int>("Channel"_ustr);
  ...
  const VArray<int> channel_attr = *attributes.lookup_or_default<int>("channel", AttrDomain::Point, -1);

  Vector<int> indices;
  for (const int i : IndexRange(src_points->totpoint)) {
    if (channel_attr[i] == channel) {
      indices.append(i);
    }
  }
  ...
  /* Copy known MIDI attributes (int). */
  auto copy_int_attribute = [&](const char *name) {
  ...
  copy_int_attribute("track");
  copy_int_attribute("channel");
  copy_int_attribute("pitch");
  copy_int_attribute("velocity");
  ...
  /* Meta event attributes (if present). */
...
static void node_register()
{
  ...
  bke::node_type_base(ntype, "GeometryNodeMidiFilterChannel"_ustr, NODE_CLASS_GEOMETRY);
  ntype.ui_name = "Filter MIDI Channel";
  ntype.ui_description = "Filter MIDI events by channel (0-15)";
  ntype.enum_name_legacy = "MIDI_FILTER_CHANNEL";
